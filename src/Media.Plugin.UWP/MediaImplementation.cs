	...
	public class MediaImplementation : IMedia
	{
		...
		public async Task<MediaFile> TakeVideoAsync(StoreVideoOptions options)
		{
			...
			if (options?.SaveToAlbum ?? false)
			{
				try
				{
					var fileNameNoEx = Path.GetFileNameWithoutExtension(options.Name ?? result.Path);
					var destination = await GetDestinationStorageFolder(options.Directory, KnownLibraryId.Videos);

					var copy = await result.CopyAsync(destination, fileNameNoEx + result.FileType, NameCollisionOption.GenerateUniqueName);
					aPath = copy.Path;
				}
				catch (Exception ex)
				{
